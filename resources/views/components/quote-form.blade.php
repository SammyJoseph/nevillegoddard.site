    ...
    <form action="{{ $action }}" method="POST">
        ...
        <div class="grid grid-cols-6 gap-4 md:gap-6 mb-6">
            <div class="relative col-span-6 md:col-span-5 order-1">
                ...
                class="w-full bg-transparent placeholder:text-slate-400 text-slate-700 dark:text-gray-200 text-sm
                        border border-slate-200 dark:border-gray-600 rounded-md px-3 py-2
                        focus:outline-none focus:border-slate-400 dark:focus:border-gray-500 hover:border-slate-300 dark:hover:border-gray-500
            ...
            <div class="col-span-6 md:col-span-4 grid grid-cols-2 gap-3 md:gap-6 order-4">
                <div class="relative">
                    <select name="source_type_id" required
                        class="w-full bg-white dark:bg-gray-700 placeholder:text-slate-400 text-slate-700 dark:text-gray-200 text-sm border border-slate-200 dark:border-gray-600 rounded pl-3 pr-8 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 dark:focus:border-gray-500 hover:border-slate-400 dark:hover:border-gray-500 shadow-sm focus:shadow-md appearance-none cursor-pointer">
        ...
        <div class="float-end flex items-center gap-3">
            ...
            @endif
            <button type="submit"
                class="flex items-center rounded-md bg-slate-800 dark:bg-blue-600 py-2 px-4 border border-transparent text-center text-sm text-white transition-all shadow-sm hover:shadow-lg focus:bg-slate-700 dark:focus:bg-blue-700 focus:shadow-none active:bg-slate-700 dark:active:bg-blue-700 hover:bg-slate-700 dark:hover:bg-blue-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
    ...
    @if(isset($method) && $method === 'PUT')
        <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex min-h-screen items-end justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0">
                ...
                <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
                    <div class="bg-white dark:bg-gray-800 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-gray-100" id="modal-title">Eliminar frase</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">¿Estás seguro de que deseas eliminar esta frase? Esta acción no se puede deshacer.</p>
                                </div>
                            </div>
                        </div>
                    </div>
