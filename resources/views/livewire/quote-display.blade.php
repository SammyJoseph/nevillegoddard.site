<div
    x-data="{ 
        ...
        resetProgressRing() {
            ...
            if (this.autoplayActive && this.isPlaying) {
    ...
    x-on:quote-refreshed.window="
        refreshAnimations();
        if (autoplayActive && isPlaying) {
            startTimer();
        }
    "
    x-on:update-autoplay.window="
        autoplayActive = $event.detail.active;
...
>
    ...
    <div class="fixed bottom-3 left-1/2 transform -translate-x-1/2 flex items-center justify-center">
        {{-- Autoplay OFF: Regular Refresh Button --}}
        <template x-if="!autoplayActive">
            <button 
                type="button" 
