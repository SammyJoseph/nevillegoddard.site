<div>
        ...
        <div class="w-full md:w-64">
            ...
            <select wire:model.live="selectedSource" id="selectedSource" class="block w-full p-4 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                @foreach ($sources as $source)
                    <option value="{{ $source->id }}">{{ $source->name }} ({{ $source->quotes_count }})</option>
                @endforeach
            </select>
        </div>
    ...
    @if(count($selectedQuotes) > 0)
        <div class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between dark:bg-red-950/20 dark:border-red-900/50 animate-fade-in-down">
            <span class="text-sm font-medium text-red-700 dark:text-red-400">
                {{ count($selectedQuotes) }} {{ count($selectedQuotes) === 1 ? 'frase seleccionada' : 'frases seleccionadas' }}
            </span>
            <x-danger-button wire:click="confirmDeleteSelected">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td class="px-6 py-4 w-12 text-center">
                        <x-checkbox wire:model.live="selectedQuotes" value="{{ $quote->id }}" />
                    </td>
    ...
    <x-confirmation-modal wire:model="confirmingQuotesDeletion">
        <x-slot name="title">
            {{ __('Confirmar eliminación') }}
        </x-slot>

        <x-slot name="content">
            {{ __('¿Estás seguro de que deseas eliminar las frases seleccionadas? Esta acción no se puede deshacer y se borrarán permanentemente.') }}
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$set('confirmingQuotesDeletion', false)" wire:loading.attr="disabled">
                {{ __('Cancelar') }}
            </x-secondary-button>
